...
// Updated LSO keys for story-1
const LSO_HEADLINE_KEY = 'story1-headline'
const LSO_PHOTO0_KEY = 'story1-photo0'
const LSO_PHOTO_KEY = 'story1-photo'
const LSO_TBQ_KEY = 'story1-tbq'
const LSO_PATH_KEY = 'story1-path'
const LSO_TBQ2_KEY = 'story1-tbq2'
const LSO_PHOTO2_KEY = 'story1-photo2'
const LSO_SHAPE_KEY = 'story1-shape'
const LSO_HS_KEY = 'story1-hs'
const LSO_PHOTO3_KEY = 'story1-photo3'
const LSO_TBQ3_KEY = 'story1-tbq3'
  ...
  useEffect(() => {
    ...
    if (savedHeadline) {
      try {
        ...
        setHeadlineCopyIdx(parsed.copyIdx ?? STORY1_DEFAULTS.headline.copyIdx)
        setHeadlineFg(parsed.fg ?? STORY1_DEFAULTS.headline.fg)
        setHeadlineBg(parsed.bg ?? STORY1_DEFAULTS.headline.bg)
      } catch (e) {
        console.error('Error parsing saved headline state:', e)
      }
    } else {
      // Use defaults
      setHeadlineCopyIdx(STORY1_DEFAULTS.headline.copyIdx)
      setHeadlineFg(STORY1_DEFAULTS.headline.fg)
      setHeadlineBg(STORY1_DEFAULTS.headline.bg)
    }
    ...
    if (savedTbq) {
      try {
        ...
        setTbqSubBodyIdx(parsed.subBodyIdx ?? STORY1_DEFAULTS.tbq.subBodyIdx)
        setTbqQuoteIdx(parsed.quoteIdx ?? STORY1_DEFAULTS.tbq.quoteIdx)
      } catch (e) {
        console.error('Error parsing saved TBQ state:', e)
      }
    } else {
      // Use defaults
      setTbqSubBodyIdx(STORY1_DEFAULTS.tbq.subBodyIdx)
      setTbqQuoteIdx(STORY1_DEFAULTS.tbq.quoteIdx)
    }
    ...
    if (savedTbq2) {
      try {
        ...
        setTbq2SubBodyIdx(parsed.subBodyIdx ?? STORY1_DEFAULTS.tbq2.subBodyIdx)
        setTbq2QuoteIdx(parsed.quoteIdx ?? STORY1_DEFAULTS.tbq2.quoteIdx)
        setTbq2ColorIdx(parsed.colorIdx ?? STORY1_DEFAULTS.tbq2.colorIdx)
      } catch (e) {
        console.error('Error parsing saved TBQ2 state:', e)
      }
    } else {
      // Use defaults
      setTbq2SubBodyIdx(STORY1_DEFAULTS.tbq2.subBodyIdx)
      setTbq2QuoteIdx(STORY1_DEFAULTS.tbq2.quoteIdx)
      setTbq2ColorIdx(STORY1_DEFAULTS.tbq2.colorIdx)
    }
    ...
    if (savedHs) {
      try {
        ...
        setHsHIdx(parsed.hIdx ?? STORY1_DEFAULTS.headlineSub.hIdx)
        setHsSIdx(parsed.sIdx ?? STORY1_DEFAULTS.headlineSub.sIdx)
        setHsAlign(parsed.align ?? STORY1_DEFAULTS.headlineSub.align as Align)
        setHsFg(parsed.fg ?? STORY1_DEFAULTS.headlineSub.fg)
        setHsBg(parsed.bg ?? STORY1_DEFAULTS.headlineSub.bg)
      } catch (e) {
        console.error('Error parsing saved HeadlineSub state:', e)
      }
    } else {
      // Use defaults
      setHsHIdx(STORY1_DEFAULTS.headlineSub.hIdx)
      setHsSIdx(STORY1_DEFAULTS.headlineSub.sIdx)
      setHsAlign(STORY1_DEFAULTS.headlineSub.align as Align)
      setHsFg(STORY1_DEFAULTS.headlineSub.fg)
      setHsBg(STORY1_DEFAULTS.headlineSub.bg)
    }
    ...
    if (savedTbq3) {
      try {
        ...
        setTbq3SubBodyIdx(parsed.subBodyIdx ?? STORY1_DEFAULTS.tbq3.subBodyIdx)
        setTbq3QuoteIdx(parsed.quoteIdx ?? STORY1_DEFAULTS.tbq3.quoteIdx)
        setTbq3ColorIdx(parsed.colorIdx ?? STORY1_DEFAULTS.tbq3.colorIdx)
      } catch (e) {
        console.error('Error parsing saved TBQ3 state:', e)
      }
    } else {
      // Use defaults
      setTbq3SubBodyIdx(STORY1_DEFAULTS.tbq3.subBodyIdx)
      setTbq3QuoteIdx(STORY1_DEFAULTS.tbq3.quoteIdx)
      setTbq3ColorIdx(STORY1_DEFAULTS.tbq3.colorIdx)
    }
    ...
    if (savedPath) {
      ...
      setPathContainer(STORY1_DEFAULTS.path)
    }
    ...
    if (savedShape) {
      try {
        ...
        setShapeSeed(parsed.seed ?? STORY1_DEFAULTS.shape.seed)
      } catch (e) {
        console.error('Error parsing saved shape seed:', e)
      }
    } else {
      // Use defaults
      setShapeSeed(STORY1_DEFAULTS.shape.seed)
    }
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key === 'l' || e.key === 'L') {
        ...
        console.log('const STORY1_DEFAULTS = ' + JSON.stringify(currentState, null, 2))
      }
    }
